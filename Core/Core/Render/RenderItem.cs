...
namespace TwiVoice.Core.Render {
    ...
    internal class RenderItem {
        ...
        private List<int> BuildPitchData(UPhoneme phoneme, UVoicePart part, UProject project) {
            ...
            if (lastNoteInvolved) {
                var offsetMs = project.TickToMillisecond(phoneme.Parent.PosTick - lastNote.PosTick);
                ...
                if (lastNote.Vibrato.Depth != 0) {
                    lastVibratoStartMs = -project.TickToMillisecond(lastNote.DurTick) * lastNote.Vibrato.Length / 100;
                    lastVibratoEndMs = 0;
                }
            }
            ...
            if (phoneme.Parent.Vibrato.Depth != 0) {
                vibratoEndMs = project.TickToMillisecond(phoneme.Parent.DurTick);
                vibratoStartMs = vibratoEndMs * (1 - phoneme.Parent.Vibrato.Length / 100);
            }

            if (nextNoteInvolved) {
                var offsetMs = project.TickToMillisecond(phoneme.Parent.PosTick - nextNote.PosTick);
            ...
            var startMs = project.TickToMillisecond(phoneme.PosTick) - phoneme.Oto.Preutter;
            var endMs = project.TickToMillisecond(phoneme.DurTick) -
            ...
            while (currMs < endMs) {
                ...
                if (currMs < lastVibratoEndMs && currMs >= lastVibratoStartMs) {
                    pit += InterpolateVibrato(lastNote.Vibrato, currMs - lastVibratoStartMs, project);
                }

                if (currMs < vibratoEndMs && currMs >= vibratoStartMs) {
                    pit += InterpolateVibrato(phoneme.Parent.Vibrato, currMs - vibratoStartMs, project);
                }
        ...
        private double InterpolateVibrato(VibratoExpression vibrato, double posMs, UProject project) {
            var lengthMs = vibrato.Length / 100 * project.TickToMillisecond(vibrato.Parent.DurTick);
