...
namespace TwiVoice.Core.Formats
{
    public static class Ust
    {
        ...
        /// <returns></returns>
        static public UProject Load(string file, string singerFolder, Encoding encoding = null)
        {
                ...
                if (line.Trim().StartsWith(@"[#") && line.Trim().EndsWith(@"]"))
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            try 
                            { 
                                currentNoteIndex = int.Parse(line.Replace("[#", string.Empty).Replace("]", string.Empty)); 
                            }
                            catch 
                            { 
                                return null; 
                ...
                else
                {
                    ...
                    if (currentBlock == UstBlock.Setting)
                    {
                        ...
                        if (line.StartsWith("VoiceDir="))
                        {
                            ...
                            //// var singers = Formats.UtauSoundbank.FindAllSingers();
                            //// var singer = UtauSoundbank.GetSinger(singerpath, FileEncoding.DetectFileEncoding(file), singers);
